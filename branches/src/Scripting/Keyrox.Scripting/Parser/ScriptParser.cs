...
namespace Keyrox.Scripting.Parser {
    public class ScriptParser {
        ...
        public void GetScriptSections() {
            ScriptSections = new List<string>();
            foreach (Row row in Document) {
                if (row.Text.StartsWith("#section")) {
                    ScriptSections.Add(row.Text.Replace("#section", "").TrimStart(' ').TrimEnd(' '));
                }
            }
        }
        #endregion
        ...
        private void Parsing() {
            ...
            foreach (Row line in Document) {
                ...
                if (parsedrow != null && !parsedrow.IgnoreRow) {
        ...
        public ScriptLine ParseRow(Row row, ScriptFile file) {
            try {
                ...
                //Primary Validation
                if (words.Count == 0) { return new ScriptLine(file, TibiaClient) { IgnoreRow = true }; }
                if (words.First().Text.StartsWith("/")) { return new ScriptLine(file, TibiaClient) { IgnoreRow = true }; }
                if (words.First().Text.StartsWith("#")) { return new ScriptLine(file, TibiaClient) { IgnoreRow = true }; }
                if (IsEmptyRow(row)) { return new ScriptLine(file, TibiaClient) { IgnoreRow = true }; }
                ...
                if (ScriptActions.ContainsKey(words.First().Text)) {
                    ...
                    var actionline = new ScriptLine(action, row.Index + 1, row.Text, file, TibiaClient);
                    ...
                    if (action.Params != null && action.Params.Length > 0) {
                        ...
                        for (int i = 0; i < args.Length; i++) {
                            if (!args[i].StartsWith("\"") && action.Params[i].NeedQuotes) { SetError(row, string.Format("The argument \"{0}\" of this function must be inside quotes. Ex. \"...\"", i.ToString())); return null; }
                            actionline.Args[i] = args[i];
                        }
                    }

                    //Waypoint Validation.
                    if (actionline.ActionName.ToLower() == "goto") {
                    ...
                    if (actionline.ActionName.ToLower() == "setparam") {
                    ...
                    foreach (var word in words) {
                        if (word.Text == "getparam") {
                            ...
                            if (!ScriptSections.Contains(row[word.Index + 2].Text)) {
